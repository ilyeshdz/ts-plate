...
test("single file at root level", () => {
  expect(emit(file("readme.md", "# Project"))).toEqual([
    { type: "file", path: "readme.md", content: "# Project" },
  ]);
});

test("single directory at root level", () => {
  expect(emit(dir("dist"))).toEqual([{ type: "dir", path: "dist" }]);
});

test("empty root produces no outputs", () => {
  expect(emit(root())).toEqual([]);
});

test("multiple roots are flattened together", () => {
  expect(emit(root(file("a.txt")), root(file("b.txt")))).toEqual([
...
test("file with undefined content omits the content field", () => {
  expect(emit(file("empty.ts"))).toEqual([{ type: "file", path: "empty.ts" }]);
});

test("file with empty string content", () => {
  expect(emit(file("empty.ts", ""))).toEqual([{ type: "file", path: "empty.ts", content: "" }]);
});
...
test("JSON object content is preserved", () => {
  ...
  expect(emit(file("package.json", pkg))).toEqual([
